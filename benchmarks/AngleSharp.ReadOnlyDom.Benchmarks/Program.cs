...
namespace AngleSharp.ReadOnlyDom.Benchmarks
{
    static class Program
    {
        static int Main(string[] args)
        {
            ...
            if (args.Length > 0 && args[0].Equals("--utf8-state-profile", StringComparison.OrdinalIgnoreCase))
            {
                return Utf8StateProfileRunner.Run(args.Skip(1).ToArray());
            }

            if (args.Length > 0 && args[0].Equals("--utf8-normalizer-fuzz", StringComparison.OrdinalIgnoreCase))
            {
