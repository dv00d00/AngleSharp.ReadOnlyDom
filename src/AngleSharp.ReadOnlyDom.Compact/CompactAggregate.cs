...
internal sealed class CompactMarkdownWriter
{
    ...
    private bool MatchesExclusion(int handle, CompactAggregateSelector selector) =>
        MatchesExclusionChain(handle, selector.Steps, selector.Steps.Length - 1, null);

    private bool MatchesExclusionChain(
        int handle,
        ReadOnlySpan<CompactAggregateSelectorStep> steps,
        int index,
        Dictionary<(int Handle, int Step), bool>? memo
    )
    {
        ...
        if (steps[index].Axis == CompactPathAxis.Child)
            return Store(parent >= 0 && MatchesExclusionChain(parent, steps, index - 1, memo));

        memo ??= new Dictionary<(int Handle, int Step), bool>();
        for (var ancestor = parent; ancestor >= 0; ancestor = _arena.Parent(ancestor))
            if (MatchesExclusionChain(ancestor, steps, index - 1, memo))
                return Store(true);
        return Store(false);
